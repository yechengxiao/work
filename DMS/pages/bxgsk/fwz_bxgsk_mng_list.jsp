...
<head>
    ...
    <script type="text/javascript">
        ...
        //参数说明：PK
        //返回值：无
        function turn_view_dialog(bxgsdm) {
            var needDispPage = document.getElementsByName("turnPage.needDispPage")[0].value
            var ts = window.showModalDialog(encodeURI("<%=baseWebPath%>/dialogue/dialog_panel.jsp?action=<%=baseWebPath%>/bxgsk/turnViewPageAction.action&bxgsk.bxgsdm=" + escape(bxgsdm) + "&turnPage.needDispPage=" + (needDispPage) + "&time=" + new Date()), window, "dialogWidth:600px;dialogHeight:300px;center:yes;help:no;status:no;resizable:no;scroll:0");
...
<form id="form1" name="form1" action="" method="post">
    ...
    <s:hidden name="module_name" value="FWZ"/>
        ...
        <table id="data_list" class="data" border="0" cellspacing="0" cellpadding="0" width="800px;">
            ...
            <s:iterator value="bxgskInfo" id="bxgsk" status="st">
                <s:if test="#st.index >= turnPage.needResultBeginNum && #st.index <= turnPage.needResultEndNum">
                    <%--数据--%>
                    <tr ondblclick="turn_view_dialog('<s:property value="#bxgsk.bxgsdm"/>')">
